...
import { FactionType, PlanetType, AIPersonality, AIDifficulty, CraftType } from '@core/models/Enums';
...
describe('AIDecisionSystem', () => {
  ...
  test('logs defend failure and invokes fallback when under attack with no homeworld', () => {
    const gameState = new GameState();
    ...
    const playerCraftId = entitySystem.createCraft(CraftType.BattleCruiser, playerPlanet.id, FactionType.Player);
    const playerCraft = gameState.craftLookup.get(playerCraftId)!;
    playerCraft.planetID = aiPlanet.id;
    gameState.rebuildLookups();
    ...
    expect(
      warnSpy.mock.calls.some(
        ([message, payload]) =>
          message === '[AI] Defend failed'
          && payload?.outcome === 'DEFEND_FAILED_NO_HOMEWORLD'
          && payload?.homeworldPresent === false
          && payload?.creditsAvailable === 0
          && payload?.defensiveActionsAttempted === 1,
      ),
    ).toBe(true);
    expect(
      logSpy.mock.calls.some(
        ([message]) => message === '[AI] Defend attempt (under attack)',
      ),
    ).toBe(true);
  ...
  test('logs defend failure for insufficient resources and invokes fallback', () => {
    const gameState = new GameState();
    ...
    expect(
      warnSpy.mock.calls.some(
        ([message, payload]) =>
          message === '[AI] Defend failed'
          && payload?.outcome === 'DEFEND_FAILED_INSUFFICIENT_RESOURCES'
          && payload?.homeworldPresent === true
          && payload?.creditsAvailable === 0
          && payload?.defensiveActionsAttempted === 1,
      ),
    ).toBe(true);
